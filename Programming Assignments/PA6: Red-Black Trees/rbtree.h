/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Haig Emirzian and Allan Joseph
 * Version     : 1.0
...
private:
    // node_ptr is the z location in the tree. We can move
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x != nullptr){
            ...
            if(key == x->key()){
                ...
                errorMessage <<"Attempt to insert duplicate key '" << key <<"'.";
                delete z;
                throw tree_exception(errorMessage.str());
            }
            if (key<x->key()){
                x = x->left;
            }
...
private:
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        else if (node->left == nullptr && node->right == nullptr && node != nullptr){ 
            //can possibly add another && - node != nullptr
            return 1;
        }
        else{
